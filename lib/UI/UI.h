...
protected:
  ...
  template <class T, typename... Args> inline T *setChild(Args &&...args) {
    reset();
  ...
  template <class T, typename... Args> inline T *addComponent(Args &&...args) {
    return static_cast<T *>(
        _components.emplace_back(std::make_unique<T>(args...)).get());
  }
...
public:
  ...
  void handleRedraw();
  bool handleTouch(uint8_t count, TouchPoint *points,
